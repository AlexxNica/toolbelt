...
interface BuilderHubClientOptions {
    availability: {
        hosts: number
        timeout: number
    },
...
interface LinkerArgs {
    appRoot: string
    buildersToStartDebuggers: string[]
}
...
export class Linker {
    ...
    public buildersToStartDebugger: string[]
    constructor(private manifest, args: LinkerArgs) {
        this.buildersToStartDebugger = args.buildersToStartDebuggers
    }


    private shouldStartDebugger() {
        const builders = this.manifest.builders
        return intersection(builders, this.buildersToStartDebugger).length > 0
    }
